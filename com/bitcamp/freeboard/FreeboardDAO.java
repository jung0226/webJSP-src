...
public class FreeboardDAO extends DBConnection{
	//글쓰기
	public int boardInserRecord(FreeboardVO vo) {
		int cnt=0;
		try {
			//1. db연결
			getConn();
	...
	public int getUpdateRecord(int no, String subject, String content) {
		...
		try {
			...
			System.out.println("글 수정 에러 발생 "+e.getMessage());
		}finally {
			getClose();			
		}
		return result;
	...
	public List<FreeboardVO> getAllRecord(int nowPage, int onePageRecord, int totalPage, int totalRecord) {
		...
		try {
			...
			String sql = "select * from"
					+ "(select * from"
					+ "(select no, subject, userid, hit, to_char(writedate, 'MM-DD HH24:MI') writedate "
			...
			if(nowPage!=totalPage) {//마지막 페이지가 아닐 때
			...
			System.out.println("전체 레코드 선택 에러발생 "+e.getMessage());
		}finally {
			getClose();
		}
		return list;
	...
	public int getDeleteRecord(int no) {
		...
		try {
			...
			System.out.println("에코드 삭제 에러 발생 "+e.getMessage());
		}finally {
			getClose();
		}		
		return result;
	...
	public int getTotalRecord() {
		...
		try {
			...
			System.out.println("총 레코드 수 구하기에서 에러발생 "+e.getMessage());
		}finally {
			getClose();
		}
		return totalRecord;
